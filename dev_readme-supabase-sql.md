...
-- =====================================================
-- 📦 ALTER TABLE: 19_songs — add size_bytes for storage usage tracking
-- =====================================================
ALTER TABLE public."19_songs" ADD COLUMN IF NOT EXISTS size_bytes BIGINT NULL;




-- =====================================================
-- 📦 TABLE: song_plays (NOT 19_-prefixed — play-tracking for playlist ranking, DEPENDS ON 19_songs AND auth.users)
-- =====================================================
CREATE TABLE IF NOT EXISTS public.song_plays (
    ...
    CONSTRAINT song_plays_song_id_fkey FOREIGN KEY (song_id) REFERENCES public."19_songs" (id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT song_plays_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users (id) ON UPDATE CASCADE ON DELETE CASCADE
) TABLESPACE pg_default;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'song_plays' AND policyname = 'Allow public read access') THEN
        CREATE POLICY "Allow public read access"
        ON public.song_plays FOR SELECT USING (true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'song_plays' AND policyname = 'Allow users to insert their own plays') THEN
        CREATE POLICY "Allow users to insert their own plays"
        ON public.song_plays FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;
END $$;
