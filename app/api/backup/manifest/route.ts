...
export async function GET(req: Request) {
  ...
  for (const song of songs ?? []) {
    if (song.song_path) {
      const size = await getFileSize("songs", song.song_path).catch(() => 0)
      files.push({ bucket: "songs", path: song.song_path, size, contentType: "audio/mpeg" })
    }
    if (includeImages && song.image_path) {
      const size = await getFileSize("images", song.image_path).catch(() => 0)
      files.push({ bucket: "images", path: song.image_path, size, contentType: "image/jpeg" })
    }
  }

  const totalBytes = files.reduce((s, f) => s + f.size, 0)
  const estimatedMs = estimateExportMs(files)
