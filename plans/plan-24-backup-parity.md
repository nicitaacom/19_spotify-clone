...
> **Status:** 🟢 Unblocked — all four questions in §1 answered. No code written yet — build order in
> §4 starts next.
...
**Answered in chat: 1️⃣, enforced by default — no toggle, no checkbox to skip it.**
...
files themselves don't move or duplicate when a song is added to a second playlist.
...
- [x] 1. Extend `getSafeStoragePath` in `libs/helpers.ts` to take the owner key, reusing the existing
      `slugifyFilePart`. Do not add a second slug function.
- [x] 2. Pass the owner key from `app/api/songs/route.ts` for both the song and the image upload.
